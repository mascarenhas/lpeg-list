...
The implementation of LPEG is based on a \emph{virtual parsing machine},
...
We also discuss the use of semantic actions in the realm of PEGs, extending PEGs
to support their use, and presenting how the virtual machine performs semantic
actions. During the matching, the machine does not actually perform the semantic
actions, but only captures enough information, so later (after the matching)
it can produce values from the captured information.
...
PEGs to support the matching of lists and describes the virtual parsing machine;
Section~\ref{sec:equivalence} proves the transformation between PEGs and their
corresponding programs in the machine is correct; Section~\ref{sec:optimizations}
...
of writing a choice between two list patterns is $\{p_1\} / \{p_2\}$. This compiles to a \textt{Choice} followed by
an \texttt{Open} instruction, followed by the compilation of $p_1$, then a \textt{Commit}, another \texttt{Open}, the compilation
...
with a list of values obtained from the captures; and \emph{fold}, which folds (or
reduces) a list of values, producing a single value.
%
\begin{table}
\centering
\begin{tabular}{|c|} \hline
...
& \tenspaces
{\frac{\Matchl{g}{p}{s}{i} \leadsto \Justc{i+j}{c}}
	{\Matchl{g}{\Pcap{simp}{p}}{s}{i} \leadsto \Justc{i+j}{(\Pcapp{simple}{s}{i}):c \Concatl \{\Pcap{close}{j}\}}}} \mylabel{simple.1} \\ \\
