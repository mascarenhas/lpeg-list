...
part of the compilation/interpretation pipeline. A PEG-based
...
optimization passes on this tree, then either compiling it to a target
language or evaluating it. Using the same abstraction (PEGs) for the
...
results are summarized on Figure~\ref{fig:microbench}, a graph showing
the running time of the benchmarks for the base LPEG, LPEG with the
...
matching only. Figure~\ref{fig:bench2} summarized the results. They
...
data~\cite{milner:ml,hutton:haskell}. The patterns are limited, not
...
Finally, parser combinator... % PORRA
